...
## 8. 개념 모델 vs 논리 모델
- 개념 모델에서 **속성이 없는 M:N** → 논리 모델에서 연결 테이블 생성
- 개념 모델에서 **속성이 있는 M:N** → 논리 모델에서 연관 엔티티로 구현

## 9. 실무 팁
- 실무에서는 대부분의 M:N 관계에 속성이 존재 → “연관 엔티티”가 거의 필수.
- 반드시 질문해야 할 핵심:
  **“이 관계에서 별도로 저장해야 하는 정보가 있는가?”**
- 의미 기반 명명 규칙을 우선 사용
  - order_item, product_review 등
- 선택적 속성이 많으면 테이블 분리 고려